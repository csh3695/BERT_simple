...
from bert_pytorch.model.utils import tester, FSPLoss
...
lr_sch = linear_lr_scheduler(start_val=1e-7, warm_step=len(dataloader)*2, warm_val=3e-5, end=len(dataloader) * 250,
    ...
    for i, data in enumerate(dataloader):
        ...
        if mask_only:
            ...
            loss = criteria(y_pred.reshape(-1, y_pred.shape[-1]), y.reshape(-1)) + \
                   kd_criteria(y_layer_res, t_layer_res)
        else:
            loss = criteria(y_pred.reshape(-1, y_pred.shape[-1]), y.reshape(-1)) + \
                   kd_criteria(y_layer_res, t_layer_res)
