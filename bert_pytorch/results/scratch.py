...
plt.savefig(f'./bert_pytorch/results/pe/sinusoidal_inner_prod.png', dpi=400)
...
plt.savefig(f'./bert_pytorch/results/pe/random_inner_prod.png', dpi=400)
...
for i in range(248):
    ...
    plt.savefig(f'./bert_pytorch/results/pe/learn_ep{i}_inner_prod.png', dpi=400)
...
bertlm.load_state_dict(torch.load(f'./bert_pytorch/model/model_saved/bert.ep{247}.mdl'))
bert = bertlm.bert
bert.eval()
...
for layer in range(12):
    ...
    for idx in range(len(attn)):
        for head in range(len(attn[0])):
            ...
            plt.savefig(f'./bert_pytorch/results/attn/attn_h{head:02}_layer{layer:02}.png', dpi=300)
